...
# copied from https://huggingface.co/AIDC-AI/Ovis2-1B/blob/main/configuration_ovis.py
""" Ovis2 configurations """
...
class ConversationFormatter(ABC):
    ...
    def __init__(self, tokenizer):
        ...
        assert (
            tokenizer_type in self.support_tokenizer_types
...
class QwenConversationFormatter(ConversationFormatter):
    ...
    def format(self, conversations: List[Dict], generation_preface=None):
        if self.gpt_token_num is None:
            self.gpt_token_num = len(
                self.tokenizer(
                    self.from2role["gpt"], add_special_tokens=False
                ).input_ids
            )

        if conversations[0]["from"] != "system":
            conversations.insert(
                0, {"from": "system", "value": self.default_system_prompt}
            )
    ...
    def format_query(self, query, generation_preface=""):
        prompt, input_ids, _ = self.format(
            [{"from": "human", "value": query}], generation_preface=generation_preface
        )
